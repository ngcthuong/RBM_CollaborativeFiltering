%% 초기화 하자
function [visHid, hidBiases, visBiases, allErr, allRMSE] = RBM_CF_Training_Batch2CD(batchdata, validate_data,  para, opts, file_name)
...
for epoch = 1:para.maxepoch
    ...
    for batch = 1 : para.numBatches         
        ...
        posHidProbs = 1./ (1 + exp(-tmp - hidBias));  
		...
		posHidProbsTemp = posHidProbs;

        %%%%% START NEGATIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for cditer = 1:opts.CD
			...
			posHidProbsTemp = 1./(1 + exp(-tmp - hidBias));
		end;	
		negHidProbs = posHidProbsTemp;
        ...
        visHidInc = momentum*visHidInc + ...
            para.epsilonW*( (posProds-negProds)/(para.numCases * 1) - para.weightCost*visHid ) ;
